...
## Visual Consistency Review — obrigatório junto com os reviews acima, a partir da Fase 14, para toda melhoria visual

Os reviews acima cobrem arquitetura, uso, demonstração, receita, confiabilidade, plataforma, marca, franquia, autonomia e impressão em 30 segundos. Este pergunta uma coisa mais estreita, mas que decide se o produto parece "desenhado por uma única mente" (ver `MANIFESTO_DO_DESIGN.md`): "esta tela reaproveita o sistema existente ou inventou o seu próprio?" A partir da Fase 14, toda mudança visual responde, com honestidade:

1. Existe coerência visual com o resto do produto (mesmo card, mesmo badge, mesmo modal)?
2. Algum padrão foi repetido em vez de recriado?
3. Existe desalinhamento (espaçamento, grid, tipografia) perceptível?
4. Existe algum componente órfão (só usado numa tela, deveria ser genérico — ou o inverso, um genérico mal encaixado)?

## Luxury Motion Review — obrigatório junto com os reviews acima, a partir da Fase 14, para toda animação/microinteração nova

Este verifica se o movimento adicionado respeita o Princípio 2 do Manifesto ("toda animação comunica estado") e o motion system já existente (`packages/design-tokens`). A partir da Fase 14, toda animação nova responde, com honestidade:

1. O movimento comunica um estado real (não é decoração)?
2. É fluido e natural (usa os easings/springs do motion system, não uma curva inventada na hora)?
3. Existe excesso de animação na mesma tela/interação?
4. `prefers-reduced-motion` é respeitado (a animação alternativa/removida ainda comunica o essencial)?

## Pixel Polish Review — obrigatório junto com os reviews acima, a partir da Fase 14, antes de qualquer entrega visual

Uma caçada literal e obsessiva por detalhes, antes de considerar qualquer tela pronta:

1. Espaçamentos inconsistentes com a escala de espaçamento do Design Language?
2. Desalinhamentos (grid, baseline, ícone vs. texto)?
3. Sombras divergentes das definidas em `globals.css` (`--shadow-subtle/elevated/premium/glow`)?
4. Radius divergente da escala (`--radius-sm/md/lg/xl/2xl/3xl/4xl`)?
5. Cores "quase iguais" que deveriam ser o mesmo token, ou o mesmo token usado para dois conceitos diferentes (viola Princípio 8)?
...
| 14 | Design System 2.0 (Front-End) | Trilha dedicada de front-end/motion/UX rodando num chat separado (esqueço backend/banco/regras de negócio). Fase A (auditoria), Fase B (Arsenal Visual Oficial) e Fase E (rollout completo) concluídas — Landing reconstruída, Login tratado, produto inteiro (Dashboard/Command Center/Playbooks/Table Map/White Label/Developer Center/Demo OS) migrado para os componentes premium, zero duplicação sobrevivendo em todo o `src/` (confirmado por varredura completa). Ver `RELATORIO_AUDITORIA_FRONTEND_FASE_A.md`, `CATALOGO_VISUAL_OFICIAL_NFC_OS.md`, `PLANO_SUBSTITUICAO_COMPONENTES.md` e ADR-058/059/060 | ✅ Concluída (dark mode fica como decisão em aberto, não incluída) |
...
## Escopo da Fase 14 (em andamento — Fase A concluída)

**Objetivo:** elevar o NFC OS ao nível visual de Stripe/Linear/Vercel/Apple em toda ponta do produto, sem redesign aleatório — cada sub-fase só avança após aprovação, nenhuma tela pode ficar "fora da família" visual. Trilha conduzida num chat dedicado exclusivamente a front-end/motion/UX (backend/banco/regras de negócio fora de escopo), mas formalizada como fase numerada do roadmap principal por decisão explícita do usuário — mesmos ADRs, mesmos 10+3 reviews de sempre.

- **Fase A (Auditoria) — concluída:** mapeamento completo de 27 rotas e ~90+ componentes compartilhados. Achado central, registrado em ADR-058: o produto **não é greenfield em design** — já existe um "NFC OS Design Language" maduro (`packages/ui`, 17 componentes premium) construído na Fase 4.5 e estendido na Fase 10, com manifesto próprio (`MANIFESTO_DO_DESIGN.md`) e motion system (`packages/design-tokens`). O problema real não é ausência de sistema, é **adoção inconsistente**: 25+ arquivos usam primitivas shadcn raw onde um componente premium equivalente já existe (ex. `organization-card.tsx` mistura os dois sistemas no mesmo arquivo), 2 componentes premium têm zero uso em produto (`PremiumPopover`, todo `skeletons.tsx`), 2 arquivos têm hex hardcoded fora de token, e a Landing pública (prioridade #1 da lista de páginas) tem 0% de adoção do Design Language. Dark mode continua com tokens prontos desde a Fase 4.5 mas nunca ligado (`forcedTheme="light"`, ADR-047 da Fase 10) — decisão explícita pendente para a Fase B. Relatório completo em `RELATORIO_AUDITORIA_FRONTEND_FASE_A.md`.
- **Fase B (Arsenal Visual Oficial) — concluída:** redefinida pelo usuário para vir antes de qualquer migração — explorar `@shadcn`/`@magicui` a fundo via shadcn MCP (registrado `@magicui` em `components.json`, já que o servidor MCP dedicado do Magic UI exige reiniciar a sessão) e congelar um catálogo com um vencedor por categoria, nunca duas opções para a mesma função. Entregue: `CATALOGO_VISUAL_OFICIAL_NFC_OS.md` (layout/motion/inputs/feedback/data/overlays/backgrounds, com rejeições explicadas — ex. `@shadcn/empty` rejeitado por já existir `EmptyState`, ripple do Magic UI rejeitado por já existir `.brand-ripple` ciente de marca), `PLANO_SUBSTITUICAO_COMPONENTES.md` (cruza a auditoria da Fase A com o catálogo, página por página, para a execução da Fase E), e 19 componentes reais instalados em `src/components/ui/` (17 Magic UI + `Spinner`/`HoverCard` do shadcn — os dois buracos genuínos que a Fase A encontrou: nenhum spinner compartilhado, nenhum hover-card). Achado técnico real durante a instalação, corrigido: todo componente Magic UI importa do pacote `motion`, diferente do `framer-motion@13.2.0` já instalado — instalar sem corrigir criaria duas bibliotecas de animação no bundle. Reescritos os imports para `framer-motion` e removido `motion`/`@radix-ui/react-icons` do `package.json` — zero dependência nova sobrevive à correção, `tsc`/`eslint` limpos. Ver ADR-059.
- **Fase C (Motion Language) e Fase D (Component Library Premium):** cobertas em boa parte pelo catálogo da Fase B (motion system já existe em `packages/design-tokens`, efeitos já catalogados). O que falta é composição, não instalação: `DatePicker`/`MultiSelect` (sobre `calendar`/`popover` já instalados), `PremiumTable`, Success/Error States, e o `AuroraBackground` próprio (nenhum registry conectado tem um de página inteira — decisão de construir com os tokens de marca em vez de adicionar um registry não verificado, ver catálogo §7).
- **Fase E (rollout página por página) — em andamento:** segue `PLANO_SUBSTITUICAO_COMPONENTES.md`, na ordem confirmada pelo usuário. **Landing — concluída:** Hero (aprovado como "régua do projeto"), Como Funciona (fluxo de 6 nós com Animated Beam, espelhando o pipeline real Ativo→Resolvedor→Destino), Bento Features (6 cards ilustrados com capacidades reais — Analytics/Heatmap/Playbooks/API/White Label/ROI, substituindo `benefits.tsx`), Prova Social (Marquee), Pricing (Border Beam no plano em destaque), FAQ, CTA final (reaproveita a linguagem visual do Hero). Achado e corrigido durante a construção: `--font-sans: var(--font-sans)` em `globals.css` era uma referência circular inválida — o produto inteiro caía para a serifa padrão do navegador, não só a Landing. **Login — tratado:** `branded-auth-screen.tsx` ganhou Aurora+Noise apenas quando não há marca própria de white label configurada (nunca sobrepõe o fundo de uma empresa cliente). **Dashboard (~20 arquivos) — migração de adoção concluída:** Configurações, Equipe, Cartões, Card Form Dialog, Access Scope Manager, painéis de API Keys/Webhooks/Logs, Ghost Mode Preview, Time Machine Panel, Campaign Builder Sheet, visão geral do Analytics, Onboarding, lista de Feedback, Assignment Manager — todos migrados de shadcn raw para os componentes premium, mais a correção do `RecommendationCard` (botões) e dos 2 hex hardcoded (branding preview + Mission Control, que ganhou o token `--noc-surface`). Verificado visualmente contra o banco de dados real semeado (Bella Vista). Achado real durante a verificação: `dashboard/layout.tsx` renderizava `<UserButton/>` do Clerk incondicionalmente, quebrando toda a árvore autenticada sob Dev Runtime — corrigido, ver ADR-060. **Segunda passada (2026-09-10) — Command Center, Playbooks, Table Map, White Label, Developer Center, Demo OS e páginas públicas revisados:** confirmado por varredura no `src/` inteiro — zero usos crus de `Card`/`Badge`/`Dialog`/`Popover` sobrevivem em qualquer lugar do produto (só infraestrutura interna do `sidebar.tsx`/`sonner.tsx`, esperado). `RecommendationCard` (compartilhado entre Command Center e `/dashboard/playbooks`) ganhou Border Beam só durante `applying=true` — o mesmo critério de sempre, nunca decorativo. Botões crus de pausar/retomar/cancelar/desfazer em `playbooks-view.tsx` convertidos para `Button`. Demo OS ganhou `EmptyState` com ação no lugar de um parágrafo manual. Table Map, White Label/Theme Studio e Developer Center já estavam limpos (nenhuma mudança necessária). Páginas públicas do fluxo de avaliação (`/r/[code]`, `/feedback`) revisadas e **deliberadamente deixadas como estão** — já são brand-aware (cor dinâmica por empresa, nunca os tokens `--brand` do produto) e já seguem o Princípio 5 do Manifesto (menos cliques, mobile, sem fricção); adicionar decoração ali contrariaria o próprio objetivo da tela. Build de produção isolado (sem servidor de dev concorrente) confirmado limpo após cada lote.

**Terceira e última passada (2026-09-10) — varredura completa até não sobrar nada:** `HoverCard` (catalogado, sem uso real até então) ganhou seu primeiro uso genuíno em `/dashboard/team` — avatar do membro no hover mostra cargo e data de entrada, verificado ao vivo contra a Bella Vista real. Varredura `grep` no `src/` inteiro confirmou zero hex hardcoded fora das exceções já documentadas (PDF export, cor de marca dinâmica, mockups vendorizados do Magic UI), zero `<button>` reimplementando o `Button`, zero `animate-spin`/`Loader2` fora do próprio `Spinner`/`sonner.tsx`. `eslint` rodado no projeto inteiro (não só nos arquivos tocados): 0 erros, só os 2 warnings pré-existentes do código vendorizado do Magic UI. Verificado ao vivo, nesta passada: Campanhas (Nova Campanha → `PremiumDrawer` sólido funcionando), Mapa de Mesas (Live Mode conectando de verdade), Command Palette (Cmd+K). Build de produção final: limpo, exit code 0. **Atualização — dark mode ligado (2026-09-10):** a decisão que ficou em aberto no fechamento anterior foi resolvida a pedido explícito do usuário ("se você achar que precisa para parecer empresa de bilionária, faça"). `forcedTheme="light"` removido, toggle real (`AnimatedThemeToggler` do Magic UI + View Transitions API, controlado pelo `next-themes`) em `SiteHeader` e no header do dashboard. Verificado ao vivo, tema escuro completo, em toda a Landing (Hero/Bento/Pricing/FAQ/CTA), Dashboard, Mapa de Mesas e Equipe — **zero ajuste de contraste foi necessário em qualquer tela**, confirmando que a disciplina de tokens semânticos mantida durante toda a Fase 14 (nunca hex hardcoded, nunca cor fora de `--brand`/`--background`/etc.) tornou dark mode trivial de ligar com segurança. Ver ADR-061. `tsc`, `eslint` (projeto inteiro) e build de produção isolado: limpos.
...
- ~~"Temas claro e escuro" da identidade de marca não foi cumprido integralmente~~ **Resolvido na Fase 14** — `forcedTheme="light"` foi removido, o toggle real existe (Landing + Dashboard), e o produto inteiro foi verificado sob tema escuro. Ver ADR-061. Registro histórico do que valia até a Fase 13: o produto nunca teve dark mode de verdade ligado em nenhuma fase (`forcedTheme="light"`; o bloco `.dark` em `globals.css` existe desde a Fase 4.5 mas nunca era aplicado). O cálculo de cor do `BrandProvider` já era agnóstico a tema — permanece o mesmo, sem mudança nesta fase; o Theme Studio (preview de marca) continua não mostrando um modo escuro próprio, um possível próximo passo, não uma pendência desta fase. Ver ADR-047 (decisão original) e ADR-061 (revogação).
